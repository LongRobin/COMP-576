...
class NeuralNetwork(object):
    ...
    def backprop(self, X, y):
        ...
        dW2 = np.transpose(self.a1) @ delta3

        db2 = np.sum(delta3, axis=0)

        dW1 = np.transpose(X) @ ((delta3 @ self.W2.T) * self.diff_actFun(self.z1, self.actFun_type))

        db1 = np.sum((delta3 @ self.W2.T) * self.diff_actFun(self.z1, self.actFun_type), axis=0)
